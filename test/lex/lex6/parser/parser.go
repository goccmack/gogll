...
import (
	...
	"github.com/goccmack/gogll/v3/test/lex/lex6/parser/bsr"
	"github.com/goccmack/gogll/v3/test/lex/lex6/parser/slot"
	"github.com/goccmack/gogll/v3/test/lex/lex6/parser/symbols"
	"github.com/goccmack/gogll/v3/test/lex/lex6/token"
)
...
func newParser(l *lexer.Lexer) *parser {
	return &parser{
		...
		crfNodes:    map[crfNode]*crfNode{},
		bsrSet:      bsr.New(symbols.NT_P1, l),
		parseErrors: nil,
	}
}
...
func (p *parser) parse() (*bsr.Set, []*Error) {
	...
	for !p.R.empty() {
		...
		case slot.P10R0: // P1 : ∙'[
			...
			p.bsrSet.Add(slot.P10R1, cU, p.cI, p.cI+1)
			p.cI++
			if p.follow(symbols.NT_P1) {
				p.rtn(symbols.NT_P1, cU, p.cI)
			} else {
				p.parseError(slot.P10R0, p.cI, followSets[symbols.NT_P1])
			}
	...
	if !p.bsrSet.Contain(symbols.NT_P1, 0, m) {
		p.sortParseErrors()
