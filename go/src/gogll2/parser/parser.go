...
func Parse(I []byte) (error, []*ParseError) {
	...
	for !R.empty() {
		...
		case slot.Head0R1: // Head : NonTerminal ∙
		...
		case slot.NonTerminal0R0: // NonTerminal : ∙letter NTChars

			bsr.Add(slot.NonTerminal0R1, cU, cI, cI+sz)
			cI += sz
			nextI, r, sz = decodeRune(I[cI:])
			if !testSelect[slot.NonTerminal0R1]() {
				parseError(slot.NonTerminal0R1, cI)
				break
			}

			call(slot.NonTerminal0R2, cU, cI)
		case slot.NonTerminal0R2: // NonTerminal : letter NTChars ∙

			if follow["NonTerminal"]() {
				rtn("NonTerminal", cU, cI)
			}
		case slot.Package0R0: // Package : ∙SepE p a c k a g e Sep String SepE

			call(slot.Package0R1, cU, cI)
		case slot.Package0R1: // Package : SepE ∙p a c k a g e Sep String SepE
			...
			bsr.Add(slot.Package0R8, cU, cI, cI+sz)
			cI += sz
			nextI, r, sz = decodeRune(I[cI:])
		...
		case slot.Package0R9: // Package : SepE p a c k a g e Sep ∙String SepE
		...
		case slot.Package0R10: // Package : SepE p a c k a g e Sep String ∙SepE
		...
		case slot.Package0R11: // Package : SepE p a c k a g e Sep String SepE ∙

			if follow["Package"]() {
				rtn("Package", cU, cI)
			}
		case slot.Rule0R0: // Rule : ∙Head SepE : SepE Alternates ;

			call(slot.Rule0R1, cU, cI)
		case slot.Rule0R1: // Rule : Head ∙SepE : SepE Alternates ;
		...
		case slot.Rule0R2: // Rule : Head SepE ∙: SepE Alternates ;
		...
		case slot.Rule0R4: // Rule : Head SepE : SepE ∙Alternates ;
		...
		case slot.Rule0R5: // Rule : Head SepE : SepE Alternates ∙;
			...
			bsr.Add(slot.Rule0R6, cU, cI, cI+sz)
			cI += sz
			nextI, r, sz = decodeRune(I[cI:])
			if follow["Rule"]() {
				rtn("Rule", cU, cI)
		...
		case slot.StringChars0R0: // StringChars : ∙not("\"")
		...
		case slot.StringChars1R0: // StringChars : ∙
			bsr.AddEmpty(slot.StringChars1R0, cI)
