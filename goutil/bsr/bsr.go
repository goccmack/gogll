...
func (b BSR) Ignore() {
	// ln, col := getLineColumn(b.LeftExtent(), set.I)
	// fmt.Printf("    bsr.Ignore %s at ln %d col %d\n", b, ln, col)
...
func (s BSR) String() string {
	rext := s.RightExtent()
	if rext-s.LeftExtent() > 60 {
		rext = s.LeftExtent() + 60
	}
	return fmt.Sprintf("%s,%d,%d,%d - %s", s.Label, s.leftExtent, s.pivot, s.rightExtent, set.I[s.LeftExtent():rext])
}
